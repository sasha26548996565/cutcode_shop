...
@section('content')
    ...
    <section>
                    ...
                    @foreach ($cart as $cartItem)
                        <tr>
                            <td scope="row" class="py-4 px-4 md:px-6 rounded-l-2xl bg-card">
                                <div class="flex flex-col lg:flex-row min-w-[200px] gap-2 lg:gap-6">
                                    <div class="shrink-0 overflow-hidden w-[64px] lg:w-[84px] h-[64px] lg:h-[84px] rounded-2xl">
                                        <img src="{{ $cartItem->product->makeThumbnail('345x320') }}" class="object-cover w-full h-full"
                            ...
                            <td class="py-4 px-4 md:px-6 bg-card">
                                <div class="font-medium whitespace-nowrap">{{ $cartItem->product->price }}</div>
                            </td>
                            <td class="py-4 px-4 md:px-6 bg-card">
                                <div class="flex items-stretch h-[56px] gap-2">
                                    <form>
                                        @csrf
                                        <button formaction="{{ route('cart.decrement', $cartItem->product) }}" formmethod="POST" type="submit"
                                        ...
                                        <input type="number"
                                            ...
                                            min="1" max="{{ $cartItem->product->count }}" value="{{ $cartItem->quantity }}" placeholder="К-во">

                                        <button formaction="{{ route('cart.increment', $cartItem->product) }}" formmethod="POST" type="submit"
                                            class="w-12 h-full rounded-lg border border-body/10 hover:bg-card/20 active:bg-card/50 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition"
                                            >+
                                        </button>
                                    </form>
                            ...
                            <td class="py-4 px-4 md:px-6 bg-card">
                                <div class="font-medium whitespace-nowrap">{{ $cartItem->price }}</div>
                            </td>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
            <div class="text-[32px] font-black">Итого: {{ $totalPrice }} ₽</div>
            <form action="{{ route('cart.clear') }}" method="POST" class="pb-3 lg:pb-0">
                @csrf
